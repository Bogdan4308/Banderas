...
local myrep, reptoday, bandcoin, Timer, credit = 0, 0, 0.000000, 0, 0
local teg = "[FamRep] "
local famchat, running = false, false
local nickrep = ""
local ip, update_link, credit_name, credit_lvl,credit_rang,credit_money, credit_feedback = '', '','','','','',''
local updateid
...
function main()
  ...
  local result, button, list, input = sampHasDialogRespond(10)
	...
	if result then
		if button == 1 then
			if credit >= 25000 then
				sampShowDialog(12, "Оформлення заявки","Ігровий нік\nРівень\nРанг у сім'ї\nСума кредиту\nЗв'язок","Вибрати","Назад",2)
			else 
	...
	if result then
		if button == 1 then
			if list == 0 then
				sampShowDialog(13, "","Введіть, будь ласка, свій нікнейм","Зберегти","",1)	
			elseif list == 1 then
				sampShowDialog(14, "","Введіть, будь ласка, свій рівень","Зберегти","",1)	
			elseif list == 2 then
				sampShowDialog(15, "","Введіть, будь ласка, свій ранг","Зберегти","",1)	
			elseif list == 3 then
				sampShowDialog(16, "","Введіть, будь ласка, суму кредиту","Зберегти","",1)	
			elseif list == 4 then
				sampShowDialog(17, "","Введіть, будь ласка, дс, тг або інше для зв'язку","Відправити","",1)	
			end
	...
	if result then
		if input ~= "" then
			credit_name = input
			sampAddChatMessage("Нік збережено", 0x7FFF00)
			sampShowDialog(12, "Оформлення заявки","Ігровий нік\nРівень\nРанг у сім'ї\nСума кредиту\nЗв'язок","Вибрати","Назад",2)
	...
	if result then
		if input ~= "" then
			credit_lvl = input
			sampAddChatMessage("Рівень збережено", 0x7FFF00)
			sampShowDialog(12, "Оформлення заявки","Ігровий нік\nРівень\nРанг у сім'ї\nСума кредиту\nЗв'язок","Вибрати","Назад",2)
	...
	if result then
		if input ~= "" then
			credit_rang = input
			sampAddChatMessage("Ранг збережено", 0x7FFF00)
			sampShowDialog(12, "Оформлення заявки","Ігровий нік\nРівень\nРанг у сім'ї\nСума кредиту\nЗв'язок","Вибрати","Назад",2)
	...
	if result then
		if input ~= "" then
			...
			if tonumber(credit_money) <= credit then
				sampAddChatMessage("Сума кредиту збережено", 0x7FFF00)
				sampShowDialog(12, "Оформлення заявки","Ігровий нік\nРівень\nРанг у сім'ї\nСума кредиту\nЗв'язок","Вибрати","Назад",2)
			else	
				credit_money = ""
				sampShowDialog(16, "","Введіть, будь ласка, суму кредиту","Зберегти","",1)
	...
	if result then
		if input ~= "" then
			...
			if credit_name == "" or credit_lvl == "" or credit_rang == "" or credit_money =="" or credit_rang == "" or credit_feedback == "" then
