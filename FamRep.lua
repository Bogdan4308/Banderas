  ...
  local result, button, list, input = sampHasDialogRespond(10)
	...
	if result then
		if button == 1 then
			...
			elseif list == 2 then
				sampShowDialog(18, "","Введіть, будь ласка, свій ігровий номер","Зберегти","Назад",1)	
			elseif list == 3 then
			...
			elseif list == 6 then
				sampShowDialog(20, "","Чи можете ви щось дати в залог?(аксесуари, нерухомість, авто або ін.)","Зберегти","Назад",1)

			elseif list == 7 then
				sampShowDialog(17, "","Введіть, будь ласка, дс, тг або інше для зв'язку","Відправити","Назад",1)
	...
	if text:find('__________Банковский чек__________') and color == 1941201407 then 
		lua_thread.create(function()
			...
			if string.match(ServerName, "X4 Payday!") then
				myrep = myrep + 4
				sampAddChatMessage(teg.."Ви отримали 4 репутації. Загальна к-ть: "..myrep, 0xFFFF00)
